...
Fixpoint addNewScoredCase {A} (l : list (A * nat)) (v : (A * nat)) :=
  match l with
  | nil => [v]
  | (bestcase, bestscore) :: l' => match v with 
    | (currentcase, currentscore) => if (gte currentscore bestscore) then
       v :: l else (bestcase, bestscore) :: (addNewScoredCase l' v)
    end
  end.

Print Checker.Result.
Definition makeCheckerResult (outcome : option bool) (message : string) : Checker.Result :=
  let m' := trace (message ++ newline) message in
  {|
    ok := outcome;
    expect := true;
    reason := m';
...
Definition giveUpCR (st : State) : Checker.Result :=
  let message := if (stDoAnalysis st) 
    then ("""result"": ""gave_up"", ""tests"":" ++ (show (numSuccessTests st)) ++ ", ""discards"": " ++ (show (numDiscardedTests st)))
    else ("*** Targeting Gave up! Passed only " ++ (show (numSuccessTests st)) ++ " tests" ++ 
      newline ++ "Discarded: " ++ (show (numDiscardedTests st)))
  in
    makeCheckerResult (Some false) message.


Definition failCR (st : State) (zero : nat): Checker.Result :=
  let numShrinks := 0 in (* TODO Shrinking *)
  let nst := numSuccessTests st in
  let ndt := numDiscardedTests st in 
  let message :=
    if stDoAnalysis st then
      ("""result"": ""failed"", ""tests"": " ++ (show (S nst)) ++ ", ""shrinks"": " ++ (show numShrinks) ++ ", ""discards"": " ++ (show ndt))%string
    else
      ("*** Targeting Failed right after " ++ (show (S nst)) ++ " tests and "
      ++ (show (numShrinks + zero)) ++ " shrinks. ("
      ++ (show ndt) ++ " discards)")%string
  in
    makeCheckerResult (Some false) message.

Definition doneTestingCR (st : State) : Checker.Result :=
  let message :=
  if (expectedFailure st) then 
    if (stDoAnalysis st) 
        then ("""result"": ""expected_failure"", ""tests"": " ++ (show (numSuccessTests st)))
        else ("*** Targeting Failed! Passed " ++ (show (numSuccessTests st))++ " tests (expected Failure)")
  else 
    if (stDoAnalysis st) 
        then ("""result"": ""success"", ""tests"": " ++ (show (numSuccessTests st)) ++ ", ""discards"": " ++ (show (numDiscardedTests st)))
        else ("+++ Targeting Passed " ++ (show (numSuccessTests st)) ++ " tests (" ++ (show (numDiscardedTests st)) ++ " discards)")
  in
    makeCheckerResult (Some true) message.

Fixpoint targetLoopAux {A} (fuel : nat) (st : State) 
         (passed : list (A * nat)) (discarded : list (A * nat))
         (strategy : State -> list (A * nat) -> list (A * nat) -> G A)
         (utility : A -> nat) (print : A -> string) 
         (prop : A -> option bool) : Checker.Result :=
  match fuel with
  | O => giveUpCR st
  | S (fuel') => 
      if (gte (numSuccessTests st) (maxSuccessTests st)) then
        doneTestingCR st (* We've succeded enough times, we're done here (good ending) *)
      else if (gte (numDiscardedTests st) (maxDiscardedTests st)) then
        giveUpCR st (* Too many discards, we're done here (bad ending) *)
      else 
        let size := (computeSize st) (numSuccessTests st) (numDiscardedTests st) in
        let (rnd1, rnd2) := randomSplit (randomSeed st) in
        let a := run (strategy st passed discarded) size rnd1 in (* strategize, then make a test case *)
        let result := prop a in (* run test *)
        let score := utility a in (* calculate utility *)
        match result with
        | Some false => 
          let z := trace (print a ++ newline) 0 in
          failCR st z
        | Some true => 
           let passed' := addNewScoredCase passed (a,score) in
           targetLoopAux fuel' (updSuccTests st S) passed' discarded strategy utility print prop
        | None => 
           let discarded' := addNewScoredCase discarded (a,score) in
           targetLoopAux fuel' (updSuccTests st S) passed discarded' strategy utility print prop
        end
      end.

Definition QuickTargetWith {A} (a : Args) (strategy : State -> list (A * nat) -> list (A * nat) -> G A)
           (utility : A -> nat) (print : A -> string) 
           (prop : A -> option bool) : Checker.Result :=
  let compFun maxSuccess maxSize n d := computeSize' a n d in
  let (rnd, computeFun) := (newRandomSeed, compFun (maxSize a) (maxSuccess a)) in
  ...
  let fuel := (maxSuccess a + maxDiscard a) in
  targetLoopAux fuel st nil nil strategy utility print prop.

Definition QuickTarget {A} := @QuickTargetWith A stdArgs.